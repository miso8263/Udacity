...
def random_test():
	...
	ret_list = [] #list of tuples
	
	q = Queue(500)
	# call enqueue and dequeue several thousand times each
	for j in range(1, 10000):
		if (random.random() < 0.5):
			rand_input = random.randint(0, 922331112)
			try:
				res = q.enqueue(rand_input) #try/catch
				err = q.checkRep() #catch assertion errors
				# For every input enqueued, add as tuple: (input, 0)
				ret_list.append((rand_input, 0))
			except:
				ret_list.append((rand_input, 1))

		else:
			try:
				res = q.dequeue()
				err = q.checkRep()
				# for every successful dequeue and checkrep, add ('dq', 0)
				ret_list.append(('dq', 0))
			except:
				ret_list.append(('dq', 1))
